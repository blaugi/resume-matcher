...
class MatchListItem(containers.VerticalGroup):
    ...
    def compose(self) -> ComposeResult:
        ...
        with containers.Horizontal(classes="match-header"):
            display_score = f"{match.original_similarity:.2f} -> {match.similarity:.2f}" if match.similarity != match.original_similarity else f"{match.similarity:.2f}"
            yield widgets.Label(f"Score: {display_score}", classes="match-score", id="match-score-label")
            yield widgets.Label(match.status, classes=f"match-status {match.status.lower().replace(' ', '-')}", id="match-status-label")
    ...
    def refresh_match_data(self) -> None:
        ...
        display_score = f"{self._match.original_similarity:.2f} -> {self._match.similarity:.2f}" if self._match.similarity != self._match.original_similarity else f"{self._match.similarity:.2f}"
        self.query_one("#match-score-label", widgets.Label).update(f"Score: {display_score}")
        status_label = self.query_one("#match-status-label", widgets.Label)
        status_label.update(self._match.status)
        
        status_label.set_classes(f"match-status {self._match.status.lower().replace(' ', '-')}")
